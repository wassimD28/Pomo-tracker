...
export const NavBar = () => {
  ...
  return (
    <div className="pointer-events-auto fixed z-50 flex h-full w-fit flex-col items-center justify-between px-4 py-8 text-custom-white-200 2xl:pl-6">
        ...
        {links.map((link) => {
          ...
          return (
            <Link
              ...
              key={link.href}
              onClick={() => handleActiveLink(link.href)}
              className={`transition-all duration-200 hover:text-white 2xl:scale-150 ${
                isActive ? "text-white" : "text-gray-400 hover:scale-110 2xl:hover:scale-[1.6]"
              }`}
            >
              <Icon size={24} />
            </Link>
      ...
      <div className="flex flex-col gap-5 2xl:scale-150">
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Image
              src={user?.imageUrl || "/default-avatar-image.jpg"}
          ...
          <DropdownMenuContent
            className="mb-4 border-none bg-custom-white-200/20 text-custom-white-200 backdrop-blur-md 2xl:scale-150 2xl:bottom-10 2xl:relative 2xl:left-14"
            side="right"
          >
            <DropdownMenuLabel>
              <div className="grid grid-cols-[35px_1fr] items-center gap-2">
